...
def get_slot_machine_spin(rows, cols, symbols):
    ...
    for _ in range(cols):
        ...
        for _ in range(rows):
            if not current_symbols:  # Safety check to avoid errors
                break
            value = random.choice(current_symbols)
            column.append(value)
            if current_symbols.count(value) > 0:  # Only remove if it exists
...
def main():
    ...
    while True:
        ...
        if balance == 0:
            ...
            choice = input("Would you like to add more money? (y/n): ").lower()
            if choice == 'y':
                balance += deposit()
            else:
